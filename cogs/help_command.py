...
class MyHelpCommand(commands.MinimalHelpCommand):
    ...
    async def _cog_select_options(self) -> nextcord.SelectOption:
        ...
        for cog, command_set in self.get_bot_mapping().items():
            ...
            options.append(nextcord.SelectOption(
                label=cog.qualified_name if cog else "Sin Categorizar",
    ...
    async def _help_embed(
        ...
        if command_set:
            ...
            for cog, command_set in mapping.items():
                ...
                if not filtered:
                    continue
                name = cog.qualified_name if cog else "🤷‍♂️ Sin Categorizar"
                emoji = getattr(cog, "COG_EMOJI", None)
...
class HelpCog(commands.Cog, name="Help"):
    """Muestra información de los comandos y cogs."""
