	...
	if Title == True:
		print(f"""{Color.Red}
		      ...
		      \:::__\\:::_ \ \::\| \| \ \::: _  \ \:::_ \ \\::::_\/\ \::\\:.\ \   
		       ...
		       /_:/____/\:\_\ \ \. \  \  \ \::(_)  \ \ \ `\ \ \:\____/\ \ \ \::\ \.
		       \_______\/\_____\/\__\/ \__\/\_______\/\_\/ \_\/\_____\/\_\/  \__\/{Color.Reset}""".replace("	", ""))
...
def PurgeCache():
	CacheFolders = JSON.Read("json/config.json")["CacheFolders"]
	for CacheFolder in CacheFolders:
		if os.name == "nt":
			CacheFolder = CacheFolder.replace("/", "\\")
			os.system(f"if exist {CacheFolder} rd /Q /S {CacheFolder}")
		if os.name == "posix": os.system(f"rm -r {CacheFolder}")
